...
export default class ProjectService extends Service {
  ...
  constructor(props: any) {
    ...
    this.ProjectValidate = {
      ...
      project_name: {
        ...
        required: true,
        trim: true,
        desc: "新增项目信息操作：项目名称不能为空"
      }
    };
  }
  // 保存用户上报的数据
  async saveProjectData(ctx: Context) {
    ...
    if (!query.app_id && type === "wx")
      return this.app.retError("新增项目信息操作：appId不能为空");
    if (!query.url && type === "web")
      return this.app.retError("新增项目信息操作：url不能为空");

    // 检验项目是否存在
    const search = await ctx.model.Project.findOne({
  ...
  // 根据用户id获取项目列表
  async getProjectsForUserId(ctx: Context) {
    const token = ctx.request.query.token;
    if (!token) return;
    const result =
      (await ctx.model.Project.where("user_id")
        .elemMatch({ $eq: token })
        .exec()) || [];
    return this.app.retResult(result);
  }
  ...
  // 删除项目中某个用户
  async deleteWebProjectUser(ctx: Context) {
    const query = ctx.request.body;
    ...
    if (!userToken)
      return this.app.retError("删除系统中某个用户：userToken不能为空");
  ...
  // 项目中新增某个用户
  async addWebProjectUser(ctx: Context) {
    const query = ctx.request.body;
    const appId = query.appId;
    const userToken = query.userToken;
  ...
  async handleDaliyEmail(
    ...
    if (item === 1) {
      handleData =
        type === 1
          ? { $addToSet: { daliy_list: email } }
          : { $pull: { daliy_list: email } };
    } else if (item === 2) {
      handleData =
        type === 1
          ? { $addToSet: { highest_list: email } }
          : { $pull: { highest_list: email } };
    }
    const result = await this.ctx.model.Project.update(
      { app_id: appId },
      handleData,
